...
These rules cover the tools that `tool-priority.md` did not originally address: QMD, LeanCtx, MemPalace, and web research. Many overlap — use the routing table to pick the right one.
...
| **Historical agent reasoning** | `MemPalace.search` | `Serena.readMemory` | — |
...
| **What did a previous agent find?** | `MemPalace.search` or `mcp__mempalace__mempalace_diary_read` |
...
| Searching for "what was decided about X" from scratch | `MemPalace.search("X")` first |
