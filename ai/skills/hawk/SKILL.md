...
7. **Code Health score:** Run `make code-health-json 2>/dev/null | .github/scripts/code-health-score.sh /dev/stdin 0` if both exist. Emit one structured finding:
   ...
   Include `top_hotspot` (worst file by finding count) in the finding description.
