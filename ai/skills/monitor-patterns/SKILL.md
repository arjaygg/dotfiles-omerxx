...
description: "Patterns for Claude Code's Monitor tool. Use when asked to watch CI, monitor GitHub Actions status, tail logs for errors, notify when a pod fails, watch a deploy, set up poll-and-diff, stream DB migration progress, or any 'tell me when X happens' task. Covers Monitor vs run_in_background vs /loop selection, grep --line-buffered discipline, 6 ready-to-use patterns (GitHub Actions, Kubernetes pods, log file tail, ArgoCD health, DB migration, generic poll-and-diff), and common buffering mistakes."
---
