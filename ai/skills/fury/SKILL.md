...
- Mutation resistance is usually achieved by strengthening assertions in normal `*_test.go` files.
- Do **not** create `*_mutation_test.go` files, placeholder mutation scaffolding, or `TODO(mutation)` stubs unless the user explicitly asks for mutation-specific files or the repo already contains maintained mutation tests that are clearly part of the requested scope.
- If you encounter untracked, autogenerated, or syntactically broken `*_mutation_test.go` files, treat that as a workspace anomaly and stop to ask before editing them.
- Never emit fake helper names or placeholder declarations such as `Testfunc ... MutationCheck`; all test code must compile and be intentional.
...
Prefer improving existing package tests over inventing new mutation-only test files.
...
- **Never generate mutation scaffolding files by default** — mutation-focused validation belongs in real tests unless the user explicitly requests otherwise
- **Never "repair" unknown untracked mutation files silently** — stop and ask first if they are not clearly part of the requested work
