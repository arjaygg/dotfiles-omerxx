...
if [[ -t 0 ]]; then
    ...
    # Use gtimeout on macOS (if available), otherwise plain cat
    if command -v timeout >/dev/null 2>&1; then
        input=$(timeout 1 cat 2>/dev/null || echo "{}")
    elif command -v gtimeout >/dev/null 2>&1; then
        input=$(gtimeout 1 cat 2>/dev/null || echo "{}")
    else
        # No timeout command available, use plain cat (will wait for EOF)
...
case "$STATUSLINE_MODE" in
    ...
    "verbose"|*)
        ...
        # Format output style display
        if [[ "$output_style" != "default" && -n "$output_style" ]]; then
            output_style_display=$'\033[36m▸\033[0m Style: \033[95m'"${output_style}"$'\033[0m '
        else
