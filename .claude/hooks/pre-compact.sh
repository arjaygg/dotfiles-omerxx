...
# Persist a fresh, generated snapshot to disk. Unlike active-context.md (hand-edited,
# can go stale or carry over from an unrelated task), this file is regenerated from
# the live transcript + git state on every compaction — it is the freshest available
# resume artifact and should take precedence if the two ever disagree.
try:
    snapshot_dir = os.path.join(cwd, 'plans')
    os.makedirs(snapshot_dir, exist_ok=True)
    with open(os.path.join(snapshot_dir, 'session-snapshot.md'), 'w', encoding='utf-8') as f:
        f.write('<!-- GENERATED by pre-compact.sh at compaction time — overwritten on every run. -->\n')
        f.write('<!-- Freshest resume artifact: derived from live transcript + git state, not hand-edited. -->\n')
        f.write('<!-- Prefer this over plans/active-context.md if the two disagree. -->\n\n')
...
# Folded from a separate PreCompact .* matcher entry (R6 —
# docs/plans/2026-07-08-reduce-context-redundancy.md). Fire-and-forget side
# effect; output shape here is {"type":"text",...} above, not
# hookSpecificOutput.additionalContext, so this is backgrounded rather than merged.
(printf '%s' "$HOOK_PAYLOAD" | bash -lc 'lean-ctx hook observe' &>/dev/null) &
