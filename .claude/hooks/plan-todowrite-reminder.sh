...
if [ "$MULTI_STEP" -eq 1 ]; then
  echo "MULTI-STEP TASK DETECTED: Use TodoWrite to track steps before starting."
  echo "Mark each item in_progress → completed as you go."
  echo "Do NOT stop until all TodoWrite items are completed."
  echo "Do NOT use TaskCreate — that spawns background agents, not a checklist."
fi
