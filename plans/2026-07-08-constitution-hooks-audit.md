...
- [x] H2 (partial, 2026-07-08): the `lean-ctx hook rewrite` PreToolUse entry — double-compressing every Bash call alongside `rtk-rewrite.sh`, contradicting decision 0004 and `.claude/LEAN_CTX.md`'s "shell hooks are NOT active" claim — removed outright from `.claude/settings.json` (not "kept if merged"; it should never have been registered). The broader fold of `advisor-escalate.sh`/`pr-title-conventional-guard.sh`/`git-commit-guard.sh`/`pre-push-remote-check.sh` into `pre-tool-gate-v2.sh` remains open, unchanged by this pass.
- [ ] Remove the duplicate standalone `lean-ctx hook observe` UserPromptSubmit entry
- [x] M3 (2026-07-08): root cause was **not** a missing symlink — every other hook in `.claude/settings.json` is registered with an absolute path straight into `~/.dotfiles/.claude/hooks/...`; `rtk-rewrite.sh` was the only one pointed at the live homedir path (`/Users/axos-agallentes/.claude/hooks/rtk-rewrite.sh`) instead, which is why it had to exist as an untracked standalone file. Fix: added the file to the tracked repo at `.claude/hooks/rtk-rewrite.sh` (content-verified byte-identical to the live copy via `diff`) and repointed `.claude/settings.json`'s hook `command` to `/Users/axos-agallentes/.dotfiles/.claude/hooks/rtk-rewrite.sh`, matching the convention every other hook already uses. No symlink needed or created. The live `~/.claude/hooks/rtk-rewrite.sh` real file becomes dead weight once this branch merges to `main` (the live `settings.json` will then point at the repo copy) — safe to delete post-merge, not before (removing it now would break the hook on `main`, which still points at it until merge).
...
- [x] M8 (2026-07-17, branch `chore/chrome-mcp-rules-cleanup`): resolved all 6 orphans. Retired (deleted, content folded elsewhere): `qmd-usage.md`, `monitor-patterns.md` (pointer facts folded into `agent-user-global.md`/`tool-priority.md`), `pctx-session-init.md` (merged into `tool-priority.md` §6). Wired in via new `@`-imports in `.claude/CLAUDE.md`: `hyper-atomic-commits.md`, `context-window-discipline.md`. Converted to a skill (situational, not baseline policy): `kubectl-efficiency.md` → `ai/skills/kubectl-efficiency/SKILL.md`. Fixed `docs/agent-configuration-architecture.md`'s false import-list claim to match. New Chrome MCP context-efficiency guidance (`ai/skills/chrome-mcp-efficiency/SKILL.md`) added the same pass, not part of the original 6 orphans — authored directly as a situational skill (not an always-on `@`-import) since browser automation isn't every session's baseline policy, same reasoning as the `kubectl-efficiency` conversion above. Paired with a PreToolUse hook (`.claude/hooks/chrome-mcp-guard.sh`) for enforcement independent of whether the skill doc is loaded.
