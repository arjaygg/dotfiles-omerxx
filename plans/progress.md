...
### Post-audit finding: `executor-implement` produced nothing on its first real use

Step 4 shipped `ai/agents/executor-implement.md` and its acceptance check was satisfied — `model:
sonnet`, `tools:` excludes `Agent`, the symlink resolves, `config-integrity.sh` exits 0. Those all
verify the **definition**. None of them verifies the agent can execute a frozen spec.

The closeout audit was its first real use. Given
`plans/specs/2026-07-28-goal05-closeout.md` — four items, each with evidence already gathered, an
explicit Do list, per-item Accepts, a Files allowlist, and a Tooling section naming the blocks it
would hit — it produced **zero commits, zero files, and no reply** to either a status ping or a
stand-down message across ~12 minutes. It then emitted three `idle_notification`s
(10:36/10:40/10:48Z) reporting itself available. The four items were completed by the Coordinator
instead; `git diff --name-only main...HEAD` confirms it left nothing behind.

Why this matters beyond one failed delegation: the whole harness assumes spec → worker → verify is
a working path, and Part VIII's ladder is about earning autonomy for exactly this kind of unattended
delegation. A worker that silently no-ops is the worst failure mode available — it is
indistinguishable from a worker that is still thinking, and the Coordinator only found out by
polling `git log`.

Not diagnosed here (out of scope for the audit, and it needs its own investigation): whether the
cause is the agent definition, the spec size, the mailbox/idle protocol, or the `Agent` tool path
for named background agents.

**Follow-up, and it is the important one:** Step 4's acceptance criteria should require a *behavioural*
check — dispatch the executor against a trivial throwaway spec and assert it commits something —
not just frontmatter assertions. As written, Step 4's box can be honestly ticked by an agent that
cannot do its job. That is the same class of defect this audit found in the lint baseline and in
`orchestrate.js`'s terminal-status test: a check that passes without the property holding.
