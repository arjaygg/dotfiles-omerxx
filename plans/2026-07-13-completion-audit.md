...
- Current branch validation: 128 Python tests, 8 maintained pre-tool fixtures, 88 Bash syntax checks plus ShellCheck baseline comparison, event/exit/structured-output fixture-contract checks, transactional staging rollback and cache-preservation coverage, hook-configuration and dead-reference baseline comparisons, workflow YAML parsing, read-only all-client bootstrap proof, sanitized signal-schema/threshold coverage, per-file and effective-chain instruction-budget checks, exact conflict checks, and `git diff --check` pass.
...
| Atomic runtime generation, backups, and clean bootstrap | **staging-only implemented** | Marked staging tree pre-renders/fsyncs targets, backs up opt-in replacements, and restores prior replacements on simulated failure; live runtime writer, migration, crash recovery, and clean-machine proof remain absent |
...
| Phase 2 multi-client generation/bootstrap | **partially implemented** | `generate`, `diff`, `doctor`, marked transactional `stage`, manifest validation, TOML rendering, read-only `setup.sh` modes, six-client/idempotency proof, and isolated unmanaged-cache preservation proof exist; mutating bootstrap, live cache behavior, and live migration remain |
