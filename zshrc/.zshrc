...
# Port 8788 — 8787 is pctx, which 404s every proxy route. `wrap` also keeps
# tool-search deferral (#746) and can restore the 1M window (--1m, #1158).
alias hclaude="HEADROOM_NO_SUBSCRIPTION_TRACKING=1 HEADROOM_CONTEXT_TOOL=lean-ctx headroom wrap claude --port 8788"
...
# Consolidated PATH - order matters: nix > homebrew > system > user tools
export PATH=/run/current-system/sw/bin:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:$HOME/.vimpkg/bin:${GOPATH}/bin:$HOME/.cargo/bin
