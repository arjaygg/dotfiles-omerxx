...
<!--
  ...
  2026-08-03 setup.sh ran `ln -sfn` here, which was wrong for the same reason
  ...
  this file's 316, so re-running setup.sh would have silently overwritten it.

  `@rules/...` below resolves against `~/.codex/rules/` in the installed
  location, which setup.sh populates with per-file symlinks into `ai/rules/`.
  Those paths are deliberately written for the live location, not for this
  repo — this file is a template, not in-repo policy. In-repo policy for
  ~/.dotfiles lives in the top-level `AGENTS.md`.
