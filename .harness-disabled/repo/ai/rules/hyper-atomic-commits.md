...
> **Precedence:** When active, these rules supersede `agent-user-global.md` for commit operations (stricter: wrapper scripts required instead of raw git).
...
`post-task-fence.sh` is retired — it lives only under `.claude/hooks/archive/` and is absent from `settings.json`'s hooks map, so it never fires today regardless of `TodoWrite`/`TaskUpdate` activity. The live commit-discipline fence on session Stop is `stop.sh`'s first-block-wins chain: `task-gate.sh` first, then the shared lifecycle adapter for a bound Claude session; only an explicitly unbound session falls back to `git-pipeline-gate.sh`. The bridge fails closed on opted-in config, adapter, binding, or controller errors. Edit/Write/MultiEdit/NotebookEdit ownership and direct Bash lifecycle mutations are gated. A detached CI watcher allows Stop immediately, as does a bound run that authored nothing — `awaiting_work` with a clean tree still on the run's declared base SHA has nothing to commit, push, or lose. Deferred merge/sync/cleanup, by contrast, blocks *every* Stop and those actions are never executed here; the "durable notice" that would release such a run after one block is not implemented, so its only exit today is `release`. Full mechanism and workflow: **`hyper-atomic-commits-reference` skill** (`ai/skills/hyper-atomic-commits-reference/SKILL.md`).
